...
| `npm run check:agreement` | Every branch of the agreement flow through the deployed UI: paper (staff upload the scan), e-signature (the student submits, staff approve), and sending either half back — undo one approval, reject that half with a reason, the student replaces only it, staff approve again. Checks it reaches a signed agreement, an open portal and exactly one booked commission. It is the hinge of the system and the hardest thing to notice going wrong: the e-signature student was locked out of the only page that could advance it, and paper agreements booked no commission at all. Takes ~6 minutes. |
