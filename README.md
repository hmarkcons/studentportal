...
| `npm run check:roles` | Multi-role staff: access is the union of roles, only a Super Admin changes anyone's roles — Management and a counsellor are refused by the database for themselves and for others (0283) — and everybody keeps at least one role. Anyone but a Super Admin sees only their own record on Staff Management, read-only, and cannot read a colleague's CNIC or address through the API, while the Super Admin reads anyone's through `staff_personal_details()` (0284, 0285). |
...
| `npm run check:pageaccess` | The menu and the pages agree on who may open what (0273): finance and a counsellor each see only their own pages, a section with nothing left in it is not shown, and typing a hidden page's address — or a detail page under it — shows the guard's refusal rather than the page. Nobody but a Super Admin edits staff: granting staff.manage to a person or a role is refused by the database (0284), a manager on Staff Management sees their own record and no one else's, and a colleague's official email cannot be changed by them directly (0274). Needs migrations 0273, 0274 and 0284. |
...
| `npm run check:visaonly` | The visa documentation and application service (0279, 0280). A counsellor setting it is refused by the database; a processing officer sets it on the Registration card, which marks the admission stages done and takes the admission-only documents off the checklist while the passport stays. Recording the admission they already hold creates a finalized application at the admitted stage with the letter filed and verified. The agreement offers only a visa-service template made through Setup and stores the visa fee with no admission charges; the invoice is that fee alone, pre-filled, every installment dated, and generate_invoice refuses an administrative charge. The Finance generator opens the student in visa mode for a counsellor who also holds Finance, and its quick edit changes the intake without touching the fee, currency or terms. Needs migrations 0279 and 0280. |
| `npm run check:builder` | The agreement template builder (0281), through the deployed Setup page as a Super Admin. A Word contract is imported: its headings, bullets and colours come across, its payment table becomes the payment chart (lending it its colours and wording) rather than a table of sample figures, its signature lines are left out, and Page & theme is set from its page and body text. A text colour, a font, a list's bullet and a theme change each reach what is saved. Preview PDF and a real student agreement generated from the template are read as PDFs: they embed the builder's fonts and draw its colours — generating runs on the server that makes real agreements, which is where a font file left out of the function shows. A template with no design still generates in the Classic fonts on A4, and choosing Classic stores no design. The staff tab has the same builder and preview. Needs migration 0281. `PORTAL_URL=http://localhost:3000` runs it against a local server. |
| `npm run check:tables` | Every table scrolls like a spreadsheet (`TableFrame`): on the leads, students, audit log, attendance, permissions, staff, staff commission, referrals and scholarship-body pages, on a laptop-sized screen, it measures that the table's window ends inside the screen and keeps doing so — growing — as the page scrolls; that scrolling down inside it leaves the heading row at its top and scrolling across leaves the row's name at its left while the next column moves; and that the scrollbars take up room rather than fading away. Then the last row's ⋮ menu on Leads, Registered students and Staff opens wholly on screen and Escape closes it, and the sidebar is the height of the screen, holds its place while the page scrolls, scrolls its menu on its own without moving the page, and shows the page you are on. All CSS and measurement, so none of it shows in a build: a wrapper that is not the scroller lets the heading scroll away, an extra overflow ancestor unfreezes the column. Read-only. `PORTAL_URL=http://localhost:3000` runs it against a local server. |
