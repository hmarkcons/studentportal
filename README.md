...
| `npm run check:catalogue` | The catalogue imports, which add or update rather than skipping what exists. Checks the three things that fail without erroring: an empty cell leaves the stored value alone (a partial sheet must not blank the columns it does not mention), a near-miss name is held back instead of overwriting the wrong row or duplicating it, and a non-Super-Admin is told the overwrite was refused rather than being shown a success — RLS (0039) refuses those UPDATEs by matching no rows, which reads as a clean success from the app. Also asserts the round trip: export a destination through /api/export/catalogue, upload it back untouched, and nothing may happen — if a clean round trip reports changes then every real edit is carrying changes nobody asked for. Works in a `zztmp` destination of its own. |
...
`check:roles`, `check:pay`, `check:studentid`, `check:catalogue`, `check:agreement` and `check:invoice` drive a browser against the deployed portal and create real (self-deleting) staff, student and agreement records in the live Supabase project, so they refuse to start without an explicit opt-in:
