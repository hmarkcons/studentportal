...
export default async function PortalDocumentsPage() {
  ...
  const { data: rawDocs } = await supabase
    ...
    .select("id, category, custom_name, status, file_path, deadline, rejected_reason, application_id")
  ...
  const docsWithUrls = await Promise.all(
    (rawDocs ?? []).map(async (d) => {
      ...
      const custom_name = `${d.custom_name ?? d.category ?? "Document"}${uni?.name ? ` — ${uni.name}` : " — General"}`;
